...
> *"Zerei"* é gíria em português pra **terminar um jogo** — de *zerar*, como em zerar tudo até o fim. Isso aqui é uma estante pra registrar todo jogo que você já zerou (e todo que ainda não).
...
**Pré-requisitos:** [.NET 8 SDK](https://dotnet.microsoft.com/download), [Node 18+](https://nodejs.org), PostgreSQL em `localhost:5432` (usuário `postgres` / senha `postgres` — ou ajuste a connection string em `backend/appsettings.json`)
...
```
Na subida, o backend roda o **seed** (gêneros/plataformas + ~50 jogos conhecidos, idempotente — pode rodar de novo sem duplicar). Também cria uma conta demo: **`demo` / `demo1234`**, já com alguns jogos registrados, pra ter algo pra ver de cara.
...
npm run dev
```
Acesse **http://localhost:5173**, entre com a conta demo (ou crie uma conta nova) e comece a registrar seus jogos.

**Testes:** `dotnet test` na raiz do repo.

### RAWG (opcional, mas recomendado)

Sem a chave, o app funciona só com o catálogo semeado localmente (capas aparecem como placeholders coloridos). Com ela, a busca importa qualquer jogo automaticamente — capa, Metacritic, nota da comunidade, DLCs e até uma seção de "jogos parecidos".

1. Crie uma conta grátis em **[rawg.io/apidocs](https://rawg.io/apidocs)** e pegue sua chave (só identifica quem está usando a API deles, sem custo).
...
Rastreadas como [issues](../../issues) em vez de uma lista aqui, pra manter a discussão num lugar só:

- [Listas de jogos por usuário](../../issues/1) — públicas/privadas, compartilháveis por imagem/CSV
- [Internacionalizar a UI](../../issues/6) (inglês)
- [Linha do tempo mensal](../../issues/3)
- [Conquistas reais por plataforma](../../issues/4) (Steam primeiro)
- [PWA / instalável no celular](../../issues/5)

Tem uma ideia ou achou um bug? Abra uma nova [issue](../../issues).

## Contribuindo

Veja [CONTRIBUTING.md](CONTRIBUTING.md) (em inglês) pro padrão de commits, checagens antes de enviar, e cuidados com segredos.
