...
public record AtualizarBibliotecaRequest(
    ...
    int? PlataformaId = null, double? Horas = null, int? Ano = null, int? Mes = null);
public record CriarJogatinaRequest(
    int? PlataformaId, int? Ano, int? Mes, double? Horas, StatusJogo Status, bool EhRejogada, string? Observacao,
    bool Zerado = false, bool Platinado = false, bool Abandonado = false);
public record JogatinaDto(
    int Id, int? PlataformaId, string? Plataforma, int? Ano, int? Mes, double? Horas, StatusJogo Status,
